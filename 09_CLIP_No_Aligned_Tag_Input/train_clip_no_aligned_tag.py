...
class TrainConfig:
    ...
    use_aligned_tag_view: bool = False
...
def build_matched_indices_cache(
    ...
    if not force and matched_indices_path.exists():
        if meta_path.exists():
            ...
            metadata = {
                ...
                "missing": -1,
                "match_ratio": -1.0,
                "removed_input": "aligned_tag_feature.npy",
            }
        return matched_indices_path, metadata
...
def print_run_settings(
    ...
    config: TrainConfig,
    alignment_metadata: dict[str, float | int | str],
) -> None:
    print("\nRun settings:")
    print(json.dumps(asdict(config), indent=2))
    print(f"data_root: {args.data_root}")
    print(f"output_dir: {args.output_dir}")
    print(f"index_cache_dir: {args.index_cache_dir}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Train the CLIP-style multi-view model without the aligned tag feature input."
    )
    parser.add_argument("--data-root", type=Path, default=Path("dataset"))
    parser.add_argument("--output-dir", type=Path, default=Path("09_CLIP_No_Aligned_Tag_Input/runs"))
    ...
    args.index_cache_dir = args.output_dir if args.index_cache_dir is None else resolve_project_path(args.index_cache_dir)
    ...
    matched_indices_path, alignment_metadata = build_matched_indices_cache(
        local_image_list=args.local_image_list,
        official_image_list=args.official_image_list,
        output_dir=args.index_cache_dir,
        force=args.force_rebuild_cache,
    )

    print_run_settings(args, config, alignment_metadata)
    print(f"Using device: {device}")
    print("Loading visual-only aligned views...")
    views, labels, matched_indices = load_training_views(args.data_root, matched_indices_path)
