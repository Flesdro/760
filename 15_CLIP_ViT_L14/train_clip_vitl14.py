...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Train multi-label classifier using CLIP ViT-L/14 features with Asymmetric Loss."
    )
    parser.add_argument("--data-root", type=Path, default=Path("dataset"))
    parser.add_argument("--output-dir", type=Path, default=Path("15_CLIP_ViT_L14/runs"))
    parser.add_argument(
        "--clip-feats-path", type=Path,
        default=Path("15_CLIP_ViT_L14/clip_vitl14_features.npy"),
        help="Pre-extracted CLIP ViT-L/14 feature array (N, 768).",
    )
    ...
    print("Loading CLIP ViT-L/14 features...")
    ...
    print(f"Feature dim: {clip_feats.shape[1]}")
    ...
    checkpoint_path = args.output_dir / "clip_vitl14_best.pt"
