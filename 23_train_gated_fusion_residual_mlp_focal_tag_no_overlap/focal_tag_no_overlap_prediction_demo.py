...
def draw_probability_figure(
    ...
    draw.text((margin, 24), "Experiment 23: Gated Fusion + Focal + No-overlap Tags", fill=(17, 24, 39), font=font)
    draw.text((margin, 54), image_path.name, fill=(75, 85, 99), font=font)
    ...
    for line_no, line in enumerate(wrap_text(gt_text, 88)[:3]):
        draw.text((margin, 680 + line_no * 18), line, fill=(31, 41, 55), font=font)

    draw.text((chart_x, 64), f"Top-{len(top_indices)} predictions", fill=(17, 24, 39), font=font)
    draw.text((chart_x, 88), f"Threshold: {threshold:g}", fill=(220, 38, 38), font=font)

    for rank, idx in enumerate(top_indices):
        y = chart_y + rank * (bar_h + gap)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run image prediction figures for experiment 23 on images_example."
    )
