...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Train multi-label classifier using CLIP ViT-B/32 visual features."
    )
    parser.add_argument("--data-root", type=Path, default=Path("dataset"))
    parser.add_argument("--output-dir", type=Path, default=Path("13_CLIP_Visual_Features/runs"))
    parser.add_argument(
        "--clip-feats-path", type=Path,
        default=Path("13_CLIP_Visual_Features/clip_visual_features.npy"),
        help="Pre-extracted CLIP feature array (N, 512).",
    )
    ...
    if clip_feats.shape[0] != labels.shape[0]:
        raise ValueError(
            ...
            "Re-run extract_clip_features.py."
        )

    # Only keep rows where CLIP actually found the image (non-zero feature vector).
    ...
    checkpoint_path = args.output_dir / "clip_visual_best.pt"
