...
DEFAULT_FEATURE_A = "Extracted_Features/BoW_int.npy"
DEFAULT_FEATURE_B = [
    "Extracted_Features/Normalized_CH.npy",
    "Extracted_Features/Normalized_CM55.npy",
    "Extracted_Features/Normalized_CORR.npy",
    "Extracted_Features/Normalized_EDH.npy",
    "Extracted_Features/Normalized_WT.npy",
]
...
class EarlyFusion(nn.Module):
    def __init__(self, dim_a: int, dim_b: int, num_classes: int, config: ModelConfig):
        super().__init__()
...
def get_original_index(num_samples: int, source: str, local_idx: int, val_ratio: float, seed: int) -> int:
...
def plot_prediction(
    ...
    fig.suptitle("ASL Early Fusion Prediction Demo", fontsize=16)
    fig.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create an image + predicted labels demo figure for ASL Early Fusion.")
    parser.add_argument("--data-root", type=Path, default=Path("dataset"), help="Folder containing Extracted_Features and labels npy files.")
    parser.add_argument("--meta-root", type=Path, default=Path("."), help="Folder containing database_img.txt and database_label.txt.")
    parser.add_argument("--image-root", type=Path, default=Path("."), help="Folder containing the images/ directory.")
    parser.add_argument("--checkpoint", type=Path, default=Path("81/E5/asl_early_fusion_concat.pt"), help="Model checkpoint path.")
    parser.add_argument("--source", choices=["train", "val"], default="val", help="Use train or validation split sample.")
    parser.add_argument("--idx", type=int, default=0, help="Local index inside the selected split.")
    parser.add_argument("--original-index", type=int, default=None, help="Original row index in database_img.txt/database features.")
    parser.add_argument("--image-path", type=Path, default=None, help="Run one local image by matching its filename/path in database_img.txt.")
    parser.add_argument("--local-images-dir", type=Path, default=None, help="Directory of local demo images to match against database_img.txt.")
    parser.add_argument("--batch-local-images", action="store_true", help="Run all images inside --local-images-dir.")
    parser.add_argument("--threshold", type=float, default=0.5)
    parser.add_argument("--top-k", type=int, default=10)
    parser.add_argument("--output", type=Path, default=Path("81/E5/asl_prediction_demo.png"))
    parser.add_argument("--class-names", type=Path, default=Path("Concepts81.txt"), help="Optional 81-line class name file.")
    ...
    if no_explicit_sample and default_images_dir.exists():
        ...
        if args.output == Path("81/E5/asl_prediction_demo.png"):
            args.output = Path("81/E5/demo_predictions")

    feature_a_path = args.data_root / DEFAULT_FEATURE_A
    feature_b_paths = [args.data_root / rel_path for rel_path in DEFAULT_FEATURE_B]
    ...
    if not args.checkpoint.exists():
        raise FileNotFoundError(
            ...
            "Run this once in ASL_baselinne.ipynb after training:\n"
            "torch.save(results['Early_Fusion (Concat)']['model'].state_dict(), '81/E5/asl_early_fusion_concat.pt')\n"
        )
    ...
    model = EarlyFusion(
        ...
        config=ModelConfig(hidden_dims=[1024, 512, 256]),
    ).to(device)
    ...
    if args.image_path is not None:
        image_key = args.image_path.as_posix()
        original_idx = image_lookup.get(image_key, image_lookup.get(args.image_path.name))
        ...
        original_idx = get_original_index(num_samples, args.source, args.idx, val_ratio=0.2, seed=42)
        image_path = resolve_image_path(args.image_root, image_paths[original_idx])

    probs = predict_probs(model, device, feature_a_path, feature_b_paths, original_idx)

    print(f"Split: {args.source}")
    print(f"Local index: {args.idx}")
    print(f"Original database index: {original_idx}")
